    ...
    protected static function booted(): void
    {
        static::creating(function (self $claim) {
            ...
            if (! filled($claim->boq_id)) {
                throw new \RuntimeException('بوq_id مطلوب لإنشاء مستخلص');
            }
            ...
            // 2) If still missing context, try to infer from BOQ
            //    (or just ensure consistency)
            $boq = null;
            try {
                $boq = Boq::query()->select(['id', 'company_id', 'branch_id', 'project_id'])->find($claim->boq_id);
            } catch (\Throwable $e) {
                // ignore here; FK will fail later if boq invalid
            }
            ...
            // 3) Auto claim_no (per company+branch+boq)
